...
private:
    ...
    //外れ値除去
    pcl::StatisticalOutlierRemoval<PointT> removal_;
    PointCloud::Ptr cloud_removal_;
    ros::Publisher pub_removal_;
...
public:
    ...
    ,pnh_("~")
    {
        ...
        //Removalフィルタ
        removal_.setMeanK(10);
        removal_.setStddevMulThresh(1.0);
        cloud_removal_.reset(new PointCloud());
        pub_removal_ = nh_.advertise<sensor_msgs::PointCloud2>("removal", 1);
    ...
    void cbPoints(const sensor_msgs::PointCloud2::ConstPtr &msg){
        try{
            ...
            //外れ値除外
            removal_.setInputCloud(cloud_voxel_);
            removal_.filter(*cloud_removal_);


            // 平面推定
            PointCloud::Ptr remaining_cloud(new PointCloud(*cloud_removal_));
            ...
            sensor_msgs::PointCloud2 output_p,output_v,output_r,output_pr;
            pcl::toROSMsg(*cloud_passthrough_, output_p);
            pcl::toROSMsg(*cloud_voxel_, output_v);
            pcl::toROSMsg(*cloud_removal_, output_r);
            pcl::toROSMsg(*remaining_cloud, output_pr);
            output_p.header = msg->header;  // ヘッダー情報をコピー
            output_v.header = msg->header; 
            output_r.header = msg->header;
            output_pr.header = msg->header;
            //パブリッシュ
            pub_passthrough_.publish(output_p);
            pub_voxel_.publish(output_v);
            pub_removal_.publish(output_r);
            pub_plane_result_.publish(output_pr);
